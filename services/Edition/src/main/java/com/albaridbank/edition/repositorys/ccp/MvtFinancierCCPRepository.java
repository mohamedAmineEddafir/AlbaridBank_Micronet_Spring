...
public interface MvtFinancierCCPRepository extends JpaRepository<MvtFinancierCCP, Long> {
     ...
     */
    List<MvtFinancierCCP> findByDateMouvementAndCodeBureauPoste(LocalDate dateMouvement, Long codeBureauPoste);
     ...
     */
    @Query("SELECT m FROM MvtFinancierCCP m WHERE m.dateMouvement = :dateMouvement AND m.codeBureauPoste = :codeBureauPoste AND ABS(m.montant) >= :montantMinimum")
     ...
     */
    @Query("SELECT COUNT(DISTINCT m.compte.idCompte), SUM(ABS(m.montant)) FROM MvtFinancierCCP m WHERE m.dateMouvement = :dateMouvement AND m.codeBureauPoste = :codeBureauPoste")
    Object[] countDistinctAccountsAndSumMontant(@Param("dateMouvement") LocalDate dateMouvement, @Param("codeBureauPoste") Long codeBureauPoste);
}